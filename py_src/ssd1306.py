...
class SSD1306:
    ...
    def fill_rect(self, x, y, w, h, col):
        ...
        Args:
            ...
            col (int): The color to fill the rectangle (1 for white, 0 for black).
        """
    ...
    def rect(self, x, y, w, h, col):
        ...
        Args:
            ...
            w (int): The width of the rectangle.
            h (int): The height of the rectangle.
            col (int): The color of the rectangle's outline (1 for white, 0 for black).
        """
        # Draw horizontal lines
        for i in range(x, x + w):
            if 0 <= i < self.width:
                if 0 <= y < self.height:
                    self.framebuf.pixel(i, y, col)
                if 0 <= y + h - 1 < self.height:
                    self.framebuf.pixel(i, y + h - 1, col)

        # Draw vertical lines
        for j in range(y + 1, y + h - 1):
            if 0 <= j < self.height:
                if 0 <= x < self.width:
                    self.framebuf.pixel(x, j, col)
                if 0 <= x + w - 1 < self.width:
                    self.framebuf.pixel(x + w - 1, j, col)
